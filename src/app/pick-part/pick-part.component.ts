...
export class PickPartComponent implements OnInit, OnDestroy {
  ...
  // Function is called from picked-parts.component.ts to update total power if user
  // deselects the item from picked parts component, so we can update which PSUs to show.
  public updatePower(power: number): void {
    // Since this function is being called by another class, I used global variables
    // to track if function has been called since class variables aren't correctly updated
    // due to scope.
    updatePowerHasBeenCalled = true;
    ...
    console.log(compatiblePSUs);
    console.log(this.tempPower);
  }
